...
*/
function totitleCase(lower_case) {    // function to convert a string into Title Case
...
*/
const friends_name = ["Sajid", "Naeem","Ashir","Zeeshan","Mumtaz"]
for(let i=0;i<friends_name.length;i++) {console.log(friends_name[i])}
...
*/
const friend_name = ["Sajid", "Naeem","Ashir","Zeeshan","Mumtaz"]
for(let i=0;i<friend_name.length;i++){
    console.log("Dear " +friend_name[i]+ " welcome to Panaverse")
}
...
*/
const vehicles = ['car', 'motorbike', 'scootie', 'truck','bicycle']

for(let i=0;i<vehicles.length;i++){
    console.log("I would like to own a Honda",vehicles[i])
}
...
*/
const guest = ["Sajid", "Naeem","Ashir","Mumtaz"]
for(let i=0;i<guest.length;i++){console.log("Dear",guest[i],"you are cordially invited to dinner")}
...
*/
for(let i=0;i<guest.length;i++){console.log("We found a bigger table Dear",guest[i])}
...
for(let i=0;i<guest.length;i++){console.log("You are invited to dinner Dear",guest[i])}
...
for(let i=0;i < len-2;i++){
    if(guest[i] != undefined){
    console.log(guest[i], "we are sorry we can only invite two people to dinner. we cannnot invite you to dinner.")        
    }
    guest.shift()
}
for (let i=0;i<guest.length;i++){    //message to each of the two people still on your list, letting them know they’re still invited
...
let subjects = ["apple", "banana", "orange"];
...
let mysubjects = ["english", "mathematics", "biology"];
...
function fCheck(fruit) {
    var text = `you really like ${fruit}s!`
    if (fruit=='banana') {
        console.log(text) ;
      } else if (fruit=='apple'){
        console.log(text);
...
*/
let userlist=['Asif','Aman','Admin','Rehan','Ashir'];
var userCheck = function(users){
    for (let i=0; i<userlist.length; i++) {
        if(userlist[i]=='Admin'){         
            ...
            console.log("Hello " +userlist[i]+ ", thank you for logging again")
        }
    }
}
...
*/
let current_users = ['Asif','Aman','Faraz','Daya','Shahmeer'] // Make a list of current users
let new_users = ['Zeeshan','Taha','aman','Ashir','Mumtaz'] // Make a list of new users

for (let i=0;i<new_users.length;i++){
    const lowercase_current_user = current_users.map(current_users => current_users.toLowerCase());
    const lowercase_new_user = new_users.map(new_users =>new_users.toLowerCase());
    if(lowercase_current_user.indexOf(lowercase_new_user[i]) == -1)
    {
...
*/
var ordinalNumber: any[] = [1,2,3,4,5,6,7,8,9]
let i = 0
while (i<ordinalNumber.length){
    if(ordinalNumber[i]==1 || ordinalNumber[i]==2 || ordinalNumber[i]==3){
        ...
        else {console.log(`${ordinalNumber[i]}rd`)}
    }
    else{
...
for (let i = 0; i < pizza.length; i++) {
  texts = 'I Like ' +pizza[i];
  console.log(texts)
}
let text2 = "I would love to eat pizza for dinner almost all the time\n"+"Mostly, I prefer chicken over other types of pizza\n"+"I really pizza!"
console.log(text2)
...
let animalList = "";
for (let i = 0; i < animals.length; i++) {console.log(animals[i])} //use a for loop to print out the name of each animal
for (let i = 0; i < animals.length; i++) {
    animalList = 'A ' +animals[i] +' would make a great pet';
...
function mak_shirt(size,text) {
    var shirttext = `Your selected shirt size is:${size} and the message on shirt is "${text}"!`
        return shirttext;
}
//test case
...
*/
function mak_album(artistName,albumTitle,tracks=0){
    var dict = {
...
function make_greater(text){
    let greatMagician = text.concat() //create a shallow copy of the original Magician's names Array
